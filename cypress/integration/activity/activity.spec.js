...
describe('GIVEN an Activity page', () => {
  ...
  it('THEN it should have all the activity data filled properly', () => {
    ...
    cy.get('[data-test=activity-form]')
      ...
      .should('have.value', 'This is to test the description in English language this is amazing')
    cy.getByName('inclusions').should('have.value', 'Inclusions')
    cy.getByName('exclusions').should('have.value', 'Exclusions')
    cy.getByName('what_to_bring').should('have.value', 'What to bring')
