...
describe('Specific checks: image upload & description inspirations', () => {
  ...
  it('country check image uploader', () => {
    ...
    cy.get('#image-library').find('[data-id]').should('have.length', 1)
    cy.get('#visible-images').find('[data-id]').should('have.length', 2)
  ...
  it('country image upload', () => {
    ...
    cy.route(
      'POST',
      'https://partners-staging.tlservers.com/content/items/c44b8127-1529-4df0-ad52-bc084f5df7ee/presigned_posts',
      'fixture:item/presigned.json'
    )
    cy.route(
      'POST',
      'https://partners-staging.**.com/content/**/attachments',
      'fixture:item/createImage.json'
    )
    ...
    cy.get('#image-library').find('.loader').should('have.length', 1)
    cy.get('#image-library').find('[data-id]').should('have.length', 2)
